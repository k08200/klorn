...
function kstStamp(now: Date): string {
  // e.g. "2026-07-06 10:00 (Monday)" in KST — the anchor for relative dates.
...
function buildPrompt(text: string, now: Date): string {
  return `Current date/time in KST (Asia/Seoul): ${kstStamp(now)}

Extract ONE calendar event from the user's utterance below. Respond with ONLY a JSON object, no prose:
{"title": string, "startTime": ISO-8601 with +09:00 offset, "endTime": ISO-8601 with +09:00 offset, "location"?: string}

Rules:
- Resolve relative dates ("내일"=tomorrow, "모레", "다음주 화요일", "tomorrow") against the current KST date above.
