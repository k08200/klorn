...
// Slice of the per-minute window only foreground may use. Without it a
// background burst (sync's summarize sweep, batch judge) fills the shared RPM
// window at the exact moment the user is waiting on an interactive draft.
export const LLM_USER_FOREGROUND_RESERVED_RPM = intEnv("LLM_USER_FOREGROUND_RESERVED_RPM", 5);
