...
// Presentational layer for the attention firewall ("Spatial Triage" design).
// Decoupled from data fetching so it can be driven by the live API
// (inbox/firewall) or by mock data (inbox/firewall-preview, for design review).
// Depth is meant to be felt before it is read: PUSH sits on a glowing,
// elevated plane; QUEUE is mid; SILENT recedes and desaturates.
...
// The class strings below encode the depth ladder for each tier.
...
// Immutable optimistic move: pull from current tier, prepend into the new one.
export function moveItemBetweenTiers(
